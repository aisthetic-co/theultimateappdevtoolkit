...
export default function CustomVideo(props: Props) {
  ...
  return (
    <div ref={videoWrapperRef} className="relative">
      {!!props.desktopVideo?.videoUrl && (
        <div className={`${props.cropVideoForMobile ? "" : "hidden md:block"}`}>
          <video
            className={`w-full ${props.cropVideoForMobile ? "h-[calc(100vh-5.2rem)] md:h-full object-cover object-center " : "h-full"}`}
            autoPlay={true}
            muted={true}
            loop
            ref={videoRef}
            ...
            controls={false}
          >
            <source src={props.desktopVideo.videoUrl} type="video/mp4" />
            Your browser does not support the video tag.
          </video>
        </div>
      )}

      {!!props.mobileVideo?.videoUrl && (
        <div className={`md:hidden`}>
          <video
            ...
            controls={false}
          >
            <source src={props.mobileVideo?.videoUrl} type="video/mp4" />
            Your browser does not support the video tag.
          </video>
        </div>
      )}

      {!isMuted && !!props.showMuteIcon && (
        <div
          onClick={handleMuteUnmute}
          className="muteIcon w-[5rem] h-[5rem] md:w-[3rem] md:h-[3rem] bg-[#ffffff21] flex justify-center items-center
            absolute right-[1.5rem] bottom-[1.5rem] rounded-[50%] cursor-pointer
          "
        >
          <svg
            className="md:w-[2rem] md:h-[1.5rem]"
            width="25"
            height="20"
            viewBox="0 0 25 20"
            fill="none"
